...
class EmployerController extends Controller
{
  ...
  public function edit($eid,$action)
	...
	public function import(){
		...
		if ($ld){
			return redirect('report-fa')->with('message',"Nhập liệu thành công");
		}else{
			return redirect('report-fa')->withErrors('message',"Nhập liệu không thành công");
	...
	public function save( Request $request)
	{
		...
		if($sqty>0) {$result= 1;} else{ $result = 0;};
		
		// add to log system`
		$rm= new Report();
		$note=" Danh sách lao động: ". implode($danhsach, ",");
		$rm->report('baotang',$result , 'nguoilaodong',DB::getPdo()->lastInsertId(),$sqty,$note);
		
		
		return redirect('report-fa')->with('message', $sqty.' người lao động lưu thành công! ');
	...
	public function update( Request $request)
	{
		...
		switch ($action){
			...
			case 'kethuctamdung': 
				...
				break;
			default:
				return redirect('report-fa')->withErrors('message',' Khai báo không thành công! ');
