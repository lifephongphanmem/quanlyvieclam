...
@section('content')
                    ...
                    <table id="sample_3" class="table table-striped table-bordered table-hover dataTable no-footer">
                        <thead>
                            <tr class="text-center">
                                ...
                                <th width="5%">CCCD/CMND</th>
                                <th width="5%">Giới tính</th>
                                <th width="5%">Xã/Phường/Thị Trấn</th>
                                <th width="5%">Thành phố/Huyện/Thị xã</th>
                                <th width="10%">Trình độ giáo dục</th>
                                <th width="10%">Tình trạng việc làm</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($model as $key => $tb)
                                <tr>
                                    ...
                                    <td>{{ $tb->huyen }}</td>
                                    <td>{{ $tb->trinhdogiaoduc != null ?$a_trinhdovanhoa[$tb->trinhdogiaoduc]:'' }}</td>
                                    <td>{{ $tb->tinhtrangvl != null ?$a_tinhtrangvl[$tb->tinhtrangvl]:'' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
    ...
    <!-- modal gửi thông báo -->
    <form method="POST" action="{{'/TimKiem/CungLaoDong/InKetQua'}}" accept-charset="UTF-8" id="frm_modify_send" target="_blank">
        @csrf
                ...
                <div class="modal-content">
                    <div class="modal-header modal-header-primary">
                        ...
                        <button type="button" data-dismiss="modal" aria-hidden="true" class="close">×</button>
                            ...
                            <input type="hidden" name="xa" value="{{$inputs['xa']}}">
                            <input type="hidden" name="huyen" value="{{$inputs['huyen']}}">
