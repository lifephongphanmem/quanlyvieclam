								...
								<div class="form-group">
									<label >Quốc tịch</label>
										...
										<?php foreach ( $countries_list as $key => $value){ ?>
											<option value='{{$key}}' <?php if($key==$ld->nation||$value==$ld->nation) echo"selected" ;?>>{{$value}}</option>
										<?php } ?>	
										</select>
								</div>
								<div class="form-group">
							...
							<div class="col-sm-4">
								...
								<div class="form-group">
									<label >Mức lương</label>
										<input type="text" name ="luong" value="{{$ld->luong}}"  class="form-control" <?php if($action!="delete"){ ?> required <?php } ?>  >						
								</div>
								<div class="form-group">
